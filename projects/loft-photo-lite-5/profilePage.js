...
export default {
  async setUser(user) {
    const photo = document.querySelector('.component-user-info-photo');
    ...
    photo.style.backgroundImage = `url('${user.photo_100}')`;
    ...
    for(const photo of photos.items){
      ...
      elem.classList.add('component-user-photo');
      elem.dataset.id = photo.id;
      elem.style.backgroundImage = `url('${size.url}')`;
      photosComp.append(elem);
    }
  },
